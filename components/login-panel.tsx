...
export function LoginPanel({
  ...
  return (
    <div className="flex flex-1 items-center justify-center p-4">
      <div className="w-full max-w-sm rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-card)] p-5 shadow-sm">
        <div className="flex rounded-md border border-[var(--border-subtle)] bg-[var(--surface-hover)] p-1">
          <AuthModeButton
            active={mode === "login"}
        ...
        {mode === "login" ? (
          <form onSubmit={onSubmit} className="mt-5 space-y-4">
            ...
            <button className="h-10 w-full rounded-md bg-[var(--accent-primary)] text-sm font-medium text-[var(--text-on-accent)] hover:bg-[var(--accent-primary-strong)]">
              Sign in
            </button>
          </form>
        ) : null}
        ...
        {mode === "reset-request" ? (
          <form onSubmit={onRequestPasswordReset} className="mt-5 space-y-4">
            ...
            <button
              ...
              className="h-9 w-full rounded-md border border-[var(--border-strong)] text-sm hover:bg-[var(--surface-hover)]"
            >
  ...
  return (
    <button
      ...
      className={`h-8 flex-1 rounded text-sm ${
        active ? "bg-[var(--surface-card)] shadow-sm" : "text-[var(--text-muted)]"
      }`}
    >
      {children}
    </button>
